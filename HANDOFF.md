...
1. **Digital gate connectivity resolution can fail with no clear cause.** On at least one real circuit, *neither* the `.net`-reuse path *nor* the wire-tracing fallback found any gate connectivity, even though the fallback is supposed to work standalone. The gates were correctly left unconverted rather than guessed at (safe failure), but the root cause of why the fallback returned nothing was not fully diagnosed before this handoff — worth investigating with that specific circuit.
