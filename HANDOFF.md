...
14. **Added an explicit upfront total to both "what's missing" listings, before any of it is individually asked about.** Both categories (library imports, device models) already printed every model+component before their own asking loop started -- this just adds a clear "Total: N component(s) across M model(s)..." line on top of that existing listing, so the full scope is obvious before diving into per-model prompts, not just implied by counting list entries. Investigated combining both categories into one true upfront total (before *either* category is asked about) but confirmed this would be actively misleading: device-model detection (the second category) genuinely can't be computed accurately before digital-gate synthesis runs, since `parse_qsch_primitive_device_refs` isn't scoped to only diodes/BJTs/MOSFETs -- it scans every non-passive component, so an unconverted LTspice gate would show up as "needing a model" in an early dry run, then silently disappear from the real listing once synthesis resolves it, contradicting the earlier total. Scoped to per-category totals instead, which are always accurate at the point they're shown. Purely additive -- no existing detection order, timing, or asking behavior changed.
