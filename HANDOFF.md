...
| `convert_startup_to_uic_with_source_ramp()` / `_build_ramp_scaler_symbol()` | Opt-in workaround for QSpice silently ignoring `.tran ... startup` — swaps it for `uic` and ramps every independent source from 0 over the same 20µs window LTspice uses, for any source waveform type. See section 6.6 for the full mechanism and the two bugs found while building it. |
...
6. Removed the "Extra .asy symbol search folders" GUI section entirely. It was functionally wired up correctly, but redundant in practice: `_default_search_paths` already searches the `.asc`'s own folder plus every standard LTspice symbol path (`~/AppData/Local/LTspice/lib/sym`, `~/Documents/LtspiceXVII/lib/sym`, ...) automatically, which is where both LTspice's own and any custom-installed `.asy` symbols conventionally live. The manual-folder UI only mattered for symbols in a genuinely non-standard location — never needed across either real circuit used this session. The equivalent `--search-path` CLI flag was left in place (opt-in, no clutter cost for a flag).
...
9. ~~**`.tran ... startup` being silently ignored can shift when a transient feature (ringing, a startup spike) appears, relative to LTspice.**~~ **Implemented as an opt-in workaround** — see 6.6 below. Structurally verified on two real circuits; not yet verified against the real QSpice simulation engine.
...
### 6.6 Alternative for `.tran ... startup` being ignored — now implemented (opt-in)
...
**Implemented as `convert_startup_to_uic_with_source_ramp()`**, gated behind a new `apply_startup_ramp_workaround` parameter on `process_models()` (off by default; exposed as a GUI checkbox and a `--apply-startup-ramp-workaround` CLI flag). It does both halves together — swaps `startup` for `uic` in the `.tran` line, and ramps **every** independent voltage/current source from 0 to its normal output over the same fixed 20µs window LTspice's own `startup` modifier uses (confirmed verbatim from LTwiki's `.TRAN Modifiers` page).

The general-case trick, which avoids reimplementing PULSE/SINE/EXP/SFFM/PWL waveform math for every source type: physically relocate each source component to a fresh, empty staging area entirely outside the schematic's existing bounding box (via the already-existing `_compute_schematic_bounds`), give its (moved) pins two freshly-labeled internal nets, then drop a small behavioral (`B`-type) source at the source's **old** position/orientation — the exact spot the rest of the circuit is already wired to — computing `V=(new_net_plus - new_net_minus)*min(1,time/20u)`. Since an ideal source's own two-terminal output *is* its intended waveform value at that instant regardless of function type, scaling it by a 0→1 ramp reproduces LTspice's "turn these sources on in the first 20µs" for any source shape, with zero waveform-formula reimplementation and zero changes to the rest of the circuit's wiring.

Two real bugs were found and fixed while building this (both caught by re-running the actual `QSpice_MCP` checker after each attempt, not assumed):
1. Relocating each source by a small fixed offset from its own position is not safe on a real circuit — even where the new pin position didn't land on an *existing wire's endpoint*, the new stub wire's own path still ran straight through the *middle* of a long, unrelated pre-existing power-rail wire, silently merging two different nets (confirmed via a real `conflicting_net_labels` checker error). Endpoint-only collision checking can't catch a wire crossing through another wire's interior — hence the "stage in genuinely empty space" approach above.
2. The new behavioral source's pin geometry was initially hardcoded to match the standard `V`-source template (local `(0,-100)`/`(0,-600)`) — wrong for a current source (`I1` in a real circuit), whose real local pins were `(0,0)`/`(0,-500)` instead, which silently disconnected it from the circuit it was supposed to inherit (confirmed via a real `floating_pin` checker warning naming exactly that pin). Fixed by building the new source's pins from the *original* component's own actual local pin coordinates instead of an assumed template.

Verified structurally clean (0 checker errors introduced) on both real circuits after both fixes, including one circuit with 32 ramped sources of mixed types (constant, PULSE, and one current source) and 8 synthesized digital gates left untouched. **Not verified against the real QSpice simulation engine** — only structural/connectivity correctness via the checker; whether the resulting waveform is actually closer to LTspice's still needs confirming in the real GUI, since no automated real-engine comparison pipeline exists yet (see 6.5).
