...
14. **Added explicit upfront totals before any missing library/model is individually asked about, at two levels.** Both categories (library imports, device models) already printed every model+component before their own asking loop started -- a "Total: N component(s) across M model(s)..." line was added on top of each existing listing, so the full scope of *that* category is obvious before diving into its per-model prompts, not just implied by counting list entries.
    ...
    On top of that, a single **combined** total across *both* categories is now shown before either one is asked about at all -- e.g. "~54 component(s) total... 17 confirmed library import(s) + ~37 device model(s) (approximate...)". The device-model half of this combined number is explicitly labeled approximate rather than exact: `parse_qsch_primitive_device_refs` isn't scoped to only diodes/BJTs/MOSFETs, it scans every non-passive component, so computing it this early (before digital-gate synthesis has run, further down) counts unconverted LTspice gates as "needing a model" even though synthesis will resolve them without ever asking about them -- confirmed directly on the real CV+CC circuit (37 estimated vs. 29 actual, the gap being exactly the gates synthesis later resolves). The real, exact per-category counts immediately below are unaffected and still accurate; only this one combined preview line carries the "approximate" caveat, stated in its own text so it's never mistaken for the confirmed number. Purely additive -- no existing detection order, timing, or asking behavior changed; the preview uses its own throwaway `QschEditor` instance read-only for counting, never touching the one the rest of the pipeline uses.
