...
12. **Added an optional "open the file now?" prompt after a successful run.** Purely additive, requested directly: after the final "Finished! Saved to: ..." confirmation (GUI) or a non-cancelled run (CLI), the user is asked whether to open the converted `.qsch` immediately, via `os.startfile()` (the OS's own default file association — QSpice if it's installed and associated). Scoped to only the two entry points (`run_gui()`/`run_cli_combined()`); `process_models()`/`convert_asc_to_qsch()` themselves are untouched, so this has no effect on anything that calls the core pipeline directly (verified: both real circuits still convert identically). Skipped entirely if generation was cancelled, since there's nothing new to open in that case.
