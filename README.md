...
**Project:** `conversion_tool.py` — converts LTspice `.asc` schematics into QSpice-compatible `.qsch` files, then fixes up every known LTspice/QSpice incompatibility so the result actually simulates.
**Deliverable:** `dist/conversion_tool.exe` (CLI + GUI, built with PyInstaller from `conversion_tool.spec`).
...
The tool has both a CLI (`conversion_tool.exe --cli ...` or run from Python) and a GUI (`conversion_tool.exe`, the default). Both drive the exact same underlying conversion/fix-up functions — nothing in the actual conversion logic depends on which interface is used.
...
All of spicelib's own bugs are worked around via **runtime patches in `conversion_tool.py`** (search for `_patch_`), not by editing the vendored copy directly — this keeps `spicelib_vendor/` diffable against a fresh upstream copy if that's ever useful.
...
**A full second performance pass** was done later in the project (profiled with `cProfile` against the real CV+CC circuit, not guessed) after the tool had accumulated enough fixes that it started feeling slow again end-to-end. Four separate, independently-verified bottlenecks were found and fixed, none of them the one above (already fixed):

1. **`AscEditor._get_symbol()` (spicelib's own internal `.asy` lookup) re-walked the filesystem on every call, with zero caching — and it's called once per *component instance*, not once per unique symbol.** On the 269-component CV+CC circuit this meant 269 full directory walks for what's actually only 18 unique symbol names. Confirmed via `cProfile`: this one call chain accounted for 93% of `convert_asc_to_qsch()`'s total runtime (13.3 of 14.3s). Fixed by caching the result per `AscEditor` instance, keyed by symbol name — confirmed safe first (the returned `AsyReader` is read-only downstream, never mutated per-component). 269 calls → 18 real lookups.
2. **`AscEditor._get_subcircuit()` had the identical bug at a second call site** — called once per subcircuit *component instance* (17 times for the CV+CC circuit's 17 op-amps), not once per the 3 unique parts they actually are. Same fix, same safety reasoning (cached per instance by `id(symbol)`, since `symbol` is already the deduplicated object from fix #1 above). 17 calls → 3 real lookups.
3. **The real, deeper cost after #1 and #2: each of those now-deduplicated unique lookups still re-walked its search directories from scratch every time**, because spicelib's `find_file_in_directory()` itself has no caching at all — it's a plain `os.walk()` per call, and it's shared infrastructure used by both fixes above. Patched at the source (`spicelib_vendor.utils.file_search.find_file_in_directory`, picked up automatically by `search_file_in_containers()` calling it as a same-module global) to build a directory → `{filename: path}` index once per directory, then answer every later lookup in that directory (for *any* filename) with an O(1) dict lookup. Same "index once, look up many" principle as `find_model_candidates()`'s own fix above, just applied to spicelib's lower-level utility. This is what actually made `convert_asc_to_qsch()` fast: 14.3s → 0.8s on the same real circuit (~18×).
4. **`_read_text_file_robust()` opened every file up to 3 times** (one open just to peek a 2-byte BOM, then a fresh open-and-read per encoding attempted) instead of reading the file once as bytes and decoding that same in-memory buffer against each candidate encoding. Same encoding order, same null-byte rejection, same final `errors="replace"` fallback — output-identical, just without the redundant disk I/O. This function is the single most-called I/O routine in `_build_model_candidate_index()`'s directory scan (thousands of calls against real library folders), so cutting ~3 opens/file down to 1 mattered directly.

**A fifth fix removed self-inflicted overhead**: `process_models()` was constructing three separate full `QschEditor(qsch_file)` parses of the same file in one run — the main editor, a reload after digital-gate synthesis (genuinely necessary, the file changed on disk), and a third, throwaway one this project's own upfront-preview feature (section 4.6 item 14) built just for counting. Confirmed nothing between the preview's original position and the main editor's original construction point writes to the file on disk, so the preview was moved to reuse the same single early-loaded editor instead of parsing a third throwaway copy — each full parse of this real circuit's `.qsch` costs ~3.6s, so this alone was a meaningful, easily-avoidable chunk of the total.

**Combined real-world result**, verified via `cProfile` + direct wall-clock timing + a full functional regression (both real circuits, exact resolved-component counts, exact injected/synthesized counts, checker output) before and after every fix above: the CV+CC circuit's full `convert_asc_to_qsch()` + `process_models()` pipeline dropped from **29.3s to 10.0s** (~2.9× faster), with `convert_asc_to_qsch()` alone dropping from **12.9s to 0.7s** (~18× faster).

**A dead-code audit was also done** (every top-level function checked for whether its name appears anywhere else in the file, then manually confirmed rather than trusting the heuristic blindly): found exactly one genuinely unused function, `_compute_schematic_bounds()` — a leftover from the general startup-ramp mechanism that was built, then fully removed at the user's direction (section 6.6); its own docstring referenced a "staging area" concept from that removed feature. Deleted. No unused imports were found. The rest of the ~5,500-line file was confirmed to have essentially no other dead code — the "too big, too slow" complaint that prompted this pass turned out to be almost entirely a runtime-performance problem (fixed above), not a code-bloat one.

**Exe size/startup was addressed too, separately from runtime performance**: `conversion_tool.spec`'s `collect_all('spicelib_vendor')` was bundling the *entire* vendored package regardless of what this tool's own code actually calls, including unused utility scripts (a step-log histogram plotter, a raw-file-to-DataFrame exporter) that pull in pandas (62MB) + scipy (111MB) + matplotlib (28MB) — over 200MB of genuinely dead weight, none of it reachable from this tool's own execution path (confirmed: the only real `scipy`/`matplotlib` usage anywhere in the vendored library is inside a step-log plotting method this tool never calls, with lazy per-function imports at that; `pandas` only appears behind its own `except ImportError` guards for optional `.to_dataframe()`-style convenience methods this tool never calls either). Excluding those three from the PyInstaller build (numpy is *not* excluded — `RawRead`'s real array-based `.raw` parsing genuinely needs it) took the built exe from 84.7MB to 25.0MB. UPX compression was also turned off: it trades a smaller file for slower startup (every bundled binary needs decompressing on load) and makes an unsigned exe look more like a malware packer to antivirus real-time scanning — a plausible contributor to the "hangs like a webpage" first-launch symptom this was aimed at, and not worth it for a tool run locally from a known folder rather than distributed as a download. Verified the trimmed exe still runs correctly end-to-end (a real subprocess run through the full CLI flow, not just the Python module).
...
14. **Added explicit upfront totals before any missing library/model is individually asked about, at two levels.** Both categories (library imports, device models) already printed every model+component before their own asking loop started -- a "Total: N component(s) across M model(s)..." line was added on top of each existing listing, so the full scope of *that* category is obvious before diving into its per-model prompts, not just implied by counting list entries.
    ...
    **Also shown as a real GUI popup, not just in the scrolling log.** `process_models()` gained one more optional hook, `preview_callback(text)`, matching the existing interface-agnostic pattern (`review_choices`, `confirm_before_generating`, ...): called once, right when the combined preview above is logged, with the same text as one block instead of line-by-line. The GUI wires this to a `messagebox.showinfo`, so the full itemized list pops up front-and-center before any prompting starts, not just scrolled past in the log widget. The CLI doesn't pass this hook at all -- its terminal output already serves the same purpose, and leaving it unset changes nothing else. Verified the callback fires exactly once with the correct combined text via a scripted stub (matching the pattern already used elsewhere in this codebase for testing GUI-hook behavior without a live Tkinter session).
...
```bash
python -m PyInstaller conversion_tool.spec --noconfirm
```
(Close any running `conversion_tool.exe` first — PyInstaller can't overwrite a locked file.)
